      ...
      <div class="relative max-w-6xl mx-auto grid lg:grid-cols-12 gap-12 items-center">
        <div class="lg:col-span-7">
          ...
          <h1 class="font-display tracking-tight leading-[1.05] mt-6 text-5xl sm:text-6xl lg:text-7xl">
            {t('home-headline-a')}
            <br />
            <span class="italic text-stone-500 dark:text-stone-400">
              {t('home-headline-b')}
            </span>
          </h1>
...
function ChatPreview() {
  const messages = [
    { name: 'mira', tone: 'bg-rose-200 text-rose-900', text: 'pulling up a fire later, who in', t: '7:42' },
    { name: 'ash', tone: 'bg-emerald-200 text-emerald-900', text: 'me. bringing the playlist', t: '7:43' },
    { name: 'jun', tone: 'bg-sky-200 text-sky-900', text: 'i made cookies. that counts as joining', t: '7:44' },
  ];
    ...
    <div class="relative">
      ...
      <div class="relative rounded-3xl bg-white/80 dark:bg-stone-900/70 backdrop-blur border border-stone-200 dark:border-stone-800 shadow-xl shadow-stone-900/5 overflow-hidden">
        <div class="flex items-center justify-between px-5 py-3 border-b border-stone-200 dark:border-stone-800 bg-stone-50/60 dark:bg-stone-900/40">
          ...
          <span class="text-xs font-mono text-stone-500 dark:text-stone-400"># the-living-room</span>
          ...
          <For each={messages}>
            {(m) => (
              <div class="flex items-start gap-3">
                <div class={`w-9 h-9 rounded-full grid place-items-center font-display text-base ${m.tone}`}>
                  {m.name[0]}
                </div>
        ...
        <For each={items}>
          {(it) => (
            <article class="group rounded-2xl bg-white/60 dark:bg-stone-900/50 border border-stone-200 dark:border-stone-800 p-7 hover:border-stone-300 dark:hover:border-stone-700 transition-colors">
              <div class={`w-11 h-11 rounded-xl grid place-items-center ${it.tint} group-hover:scale-105 transition-transform`}>
          ...
          <For each={stats}>
            {(s) => (
              <div class="rounded-2xl border border-stone-200 dark:border-stone-800 bg-white/50 dark:bg-stone-900/40 p-6">
                ...
                <dd class="mt-2 text-xs uppercase tracking-wider text-stone-500 dark:text-stone-400">{s.label}</dd>
              </div>
            )}
          </For>
        </dl>
    ...
    <footer class="px-6 lg:px-12 py-10 border-t border-stone-200/70 dark:border-stone-800/70">
      <div class="max-w-6xl mx-auto flex flex-wrap items-center justify-between gap-4 text-sm text-stone-500 dark:text-stone-400">
        ...
        <div class="flex items-center gap-5">
          <Link to="/login" class="hover:text-stone-900 dark:hover:text-stone-100 transition-colors">
            sign in
          </Link>
          <Link to="/register" class="hover:text-stone-900 dark:hover:text-stone-100 transition-colors">
            register
          </Link>
        </div>
      </div>
